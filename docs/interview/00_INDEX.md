...
면접이 임박했다면 `05_cheatsheet.md`만 읽어도 된다. 논문 13편 한 줄 요약, 프로젝트 30초 스크립트, 반드시 외울 숫자가 압축돼 있다.
...
| 경로 | 책임 |
|------|------|
| `00_INDEX.md` | 이 문서. 전체 지도, 학습 순서, D-day 플랜 |
| `01_foundations/01_통계·머신러닝_기초.md` | 회귀·정규화·편향분산·트리앙상블·PLS-DA·평가지표 |
| `01_foundations/02_딥러닝·CNN_기초.md` | 역전파·옵티마이저·BN·백본 계보·전이학습·경량화 |
| `01_foundations/03_비전_검출·분할·키포인트.md` | YOLO·IoU/NMS/mAP·세그멘테이션·준지도·HRNet |
| `01_foundations/04_3D·PointCloud·LiDAR.md` | LiDAR 원리·깊이맵·카메라 파라미터·역투영·정합·B-Rep |
| `01_foundations/05_생성모델_GAN·Diffusion.md` | GAN·pix2pix/CycleGAN·DDPM·조건부 생성·생성 평가지표 |
| `01_foundations/06_강화학습·시계열·기타.md` | MDP/DQN/Replay·시계열 기술지표·블록체인/SBT/IPFS·LLM 기초 |
| `02_projects/P1_모바일LiDAR_3차원계측.md` | 모바일 LiDAR 기반 3차원 계측 및 속성 추정 (2023.01–2026.06) |
| `02_projects/P2_가스배관_센서탐지.md` | 로봇 센서 데이터 기반 가스배관 변화구간 탐지 (2022.01–2023.01) |
| `02_projects/P3_상품추천_멀티모달.md` | 시각 이미지와 메타데이터 통합 상품 추천 (2021.01–2022.01) |
| `02_projects/P4_영상기반_미세먼지추정.md` | 영상 기반 미세먼지 농도 추정 (2020.01–2021.01) |
| `02_projects/P5_국가RnD과제·특허.md` | 국가 R&D 과제 8건 + 특허 2건 |
| `03_papers/T01_상품추천_멀티모달.md` | 논문 T01 (제1저자) |
| `03_papers/T02_전후면의류_확산모델.md` | 논문 T02 (교신저자) |
| `03_papers/T03_모바일LiDAR_인체계측.md` | 논문 T03 |
| `03_papers/T04_의류치수_포인트클라우드.md` | 논문 T04 |
| `03_papers/T05_딸기_크기무게추정.md` | 논문 T05 |
| `03_papers/T06_NeuroBRep_CAD복원.md` | 논문 T06 |
| `03_papers/T07_멀티뷰앙상블_준지도분할.md` | 논문 T07 |
| `03_papers/T08_치과영상_GAN생성.md` | 논문 T08 |
| `03_papers/T09_미세먼지_디헤이징.md` | 논문 T09 |
| `03_papers/T10_강화학습_이중리플레이.md` | 논문 T10 |
| `03_papers/T11_헤드앤숄더_투자모델.md` | 논문 T11 |
| `03_papers/T12_SBT_분산스토리지.md` | 논문 T12 |
| `03_papers/T13_SBT_복구메커니즘.md` | 논문 T13 |
| `04_qa/01_기초개념_QA.md` | 기초 개념 질문 80문항 이상 |
| `04_qa/02_프로젝트_QA.md` | 프로젝트 질문 80문항 이상 |
| `04_qa/03_논문_QA.md` | 논문 질문 150문항 이상 |
| `04_qa/04_기여도·공저자_QA.md` | 기여도 압박 질문 25문항 이상 |
| `04_qa/05_인성·직무·커리어_QA.md` | 인성·직무 질문 40문항 이상 |
| `04_qa/06_역질문_리스트.md` | 면접 끝에 던질 역질문 20문항 이상 |
| `05_cheatsheet.md` | 전날 1시간 압축본 |
...
`04_qa/` 6종을 처음부터 끝까지 소리 내어 답한다. 막히는 질문에 표시하고, 해당 원 문서로 돌아가 그 부분만 다시 읽는다. 특히 `04_기여도·공저자_QA.md`의 빈칸은 이 시점까지 반드시 채워 둔다.

### D-1 — 치트시트만

`05_cheatsheet.md`를 세 번 읽는다. 새 내용을 넣지 않는다. `## 반드시 외울 숫자`와 `## 답하기 어려운 질문 Top 10`에 시간을 쓴다.
...
`06_역질문_리스트.md`만 이 형식을 다르게 쓴다. 그쪽은 내가 면접관에게 던지는 질문이라, `**의도:**`가 "이 질문으로 무엇을 알아내려는가", `**답변:**`이 "돌아올 답을 어떻게 해석할 것인가"가 된다.
